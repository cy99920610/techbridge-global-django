...
from core.models import CompanySettings, FAQ, ProcessStep, ServiceCategory, ServiceItem, Testimonial


class Command(BaseCommand):
    help = 'Load starter content for the public website and admin panel.'
    ...
    def handle(self, *args, **options):
        CompanySettings.objects.get_or_create(
            ...
            defaults={
                ...
                'hero_subtitle': (
                    'We provide software development, IT consulting, high-tech recruitment '
                    'and relocation coordination for companies expanding internationally.'
                ),
            },
        )
        ...
        for order, (icon, title, desc, items) in enumerate(data, start=1):
            category, _ = ServiceCategory.objects.get_or_create(
                title=title,
                defaults={'icon': icon, 'description': desc, 'order': order},
            )
            for idx, (item_title, item_desc) in enumerate(items, start=1):
                ServiceItem.objects.get_or_create(
                    category=category,
                    title=item_title,
                    defaults={'description': item_desc, 'order': idx},
                )
        ...
        Testimonial.objects.get_or_create(
            ...
            defaults={
                'client_role': 'B2B Technology Partner',
                'quote': 'TechBridge Global provides a clear and professional structure for technology delivery and international talent support.',
            },
        )
